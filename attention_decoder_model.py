...
def double_attention_decoder(decoder_inputs,
    ...
    with variable_scope.variable_scope(
        ...
        hidden = array_ops.reshape(attention_states,
                                   [-1, attn_length, 1, attn_size])
        source_emb_encoder_inputs = array_ops.reshape(source_emb_encoder_inputs,
                                    [-1, attn_length, 1, attn_size])
...
#                     hidden_source = hidden + source_emb_encoder_inputs
                ...
                with variable_scope.variable_scope("Attention_decode_%d" % a):
                    ...
                    de_d = math_ops.reduce_sum(array_ops.reshape(de_a, [-1, tf.shape(decoder_state)[1], 1]) * decoder_state , 
                                          [1])
